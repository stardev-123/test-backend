...
const { API_KEY, API_SECRET, demo } = config.tradeblock

const tradeblock = require('tradeblock-node')
const constants = require('../../../lib/constants')
const investmentManager = require('../../investmentManager')

const tradeAPI = tradeblock.init({ API_KEY, API_SECRET, demo })
...
const _acceptSellQuickTrade = async (userId, investmentId, asset, single, assetTrade, req) => {
  ...
  const result = await tradeAPI.acceptQuickTrade({
  ...
  if (result && result.trades && result.trades.length > 0) {
    ...
    //
    // })
    const trade = await tradeAPI.getTrade(result.trades[0].trade_id)
...
exports.buyCurrencies = async ({ id, assets, userId, single }, coinPrices, req) => {
  ...
  const quickTrades = await tradeAPI.listQuickTrades(currencies)
...
exports.getCurrentTradePrices = async (coins, req) => {
  ...
  const quickTrades = await tradeAPI.listQuickTrades(coins)
  if (!quickTrades || !Array.isArray(quickTrades)) {
    throw error('CRYPTO_SERVICE_IS_DOWN', quickTrades)
  }

  return quickTrades.map(trade => ({
    currency: _convertFromTradeBlockSymbol(trade.asset),
